* Paris_tour_guide.py : Using OpenAI to create a virtual Paris tour guide to help tourists answering questions and planning tours
* Embeddings.py : Using OpenAI to get the embeddings of the input string
* Finding_the_most_similar_product.py : Using OpenAI to perform semantic search 
* Product_recommendation_system.py : Using OpenAI to implement a product recommendation system
* Embeddings_for_classification_tasks.py : Using OpenAI to implement a sentiment analysis for restaurant reviews
* Using_chromadb_for_movie_embedding.py : Using OpenAI and chromadb to implement vector database for a movie embedding 
* Querying_the_Netflix_collection.py : Using OpenAI and chromadb to implment a netflix vector database for a movie query 
* Querying_the_Netflix_collection_with_multiple_texts.py : Using OpenAI and chromadb to implment a netflix vector database for multiple movie queries and with metadatas query
* Topic_Analysis_of_Clothing_Reviews_with_Embeddings.py : Using OpenAI to analyze reviews from the Women's Clothing E-Commerce Reviews dataset
* Semantic_search_engine_using_Pinecone.py : Using OpenAI and vector database, Pinecone, to implement a semantic search engine
* RAG_chatbot_with_Pinecone.py : Using OpenAi and vector database, Pinecone, to implement a RAG chatbot

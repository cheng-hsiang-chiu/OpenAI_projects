# Medical professionals often summarize patient encounters in transcripts written in natural language, which include details about symptoms, diagnosis, and treatments. 
# These transcripts can be used for other medical documentation, such as for insurance purposes,
# but as they are densely packed with medical information, extracting the key data accurately can be challenging.

# We leverage the OpenAI API to automatically extract medical information from these transcripts and automate the matching with the appropriate ICD-10 codes. 
# ICD-10 codes are a standardized system used worldwide for diagnosing and billing purposes, such as insurance claims processing.

# The dataset contains anonymized medical transcriptions categorized by specialty.

# transcriptions.csv has the following format
#    Column	          |  Description
...
def extract_info_with_openai(transcription):
  ...
  # Define the messages to the openAI   
  messages = [
        {
            "role": "system",
            ...
            'function': {
                ...
                'parameters': {
                    ...
                    'properties': {
                        ...
                        'Recommended Treatment/Procedure': {'type': 'string', 'description': 'Recommended treatment or procedure for the patient'}
                    }
                }
            }
        }
...
df_structured = pd.DataFrame(output_data)
